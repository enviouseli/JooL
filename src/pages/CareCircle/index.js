...
import { AntDesign, Entypo, Ionicons, FontAwesome5, Feather } from "@expo/vector-icons";
...
export default function CareCircle({ navigation }) {
  ...
  const handleSearch=(query)=>{
    ...
    const filteredData=fullData.filter(carebuddy=>carebuddy.language.includes(query)||
    carebuddy.location.includes(query)||carebuddy.dicipline.includes(query))
  ...
  return (
      ...
      <ScrollView horizontal={true} >
        <TagButtons text="#heart" page={() => {searchByTag("#heart")}}/>
        <TagButtons text="#teeth" page={() => {searchByTag("#teeth")}}/>
        <TagButtons text="#blood pressue" page={() => {searchByTag("#blood pressue")}}/>
        <TagButtons text="#mouth" page={() => {searchByTag("#mouth")}}/>
        <TagButtons text="#knees" page={() => {searchByTag("#knees")}}/>
      </ScrollView>

      <ScrollView horizontal={true} style={{marginVertical:5}}>
          <SmallButtons text="CareBuddy"
          icon=<FontAwesome5 name='user' size={26} color='#0A2249'/>
          page={() => {searchByDiciplain("CareBuddy")}}
          bgc="#FFE2DC" font_size={12}
          />
          <SmallButtons text="Psychologist"
          icon=<FontAwesome5 name='user' size={26} color='#0A2249'/>
          page={() => {searchByDiciplain("Psychologist")}}
          bgc="#CECCCC" font_size={12}
          />
          <SmallButtons text="Social Worker"
          icon=<FontAwesome5 name='user' size={26} color='#0A2249'/>
          page={() => {searchByDiciplain("Social Worker")}}
          bgc="#E0EAF9" font_size={12}
          />
          <SmallButtons text="Therapist"
          icon=<FontAwesome5 name='user' size={26} color='#0A2249'/>
          page={() => {searchByDiciplain("Therapist")}}
          bgc="#FFF8E2" font_size={12}
          />
        </ScrollView>
        ...
        renderItem={({item, index})=>(
          ...
          <View style={{flexDirection:"row"}}>
            <TouchableOpacity style={[Style.userBox,Style.shadowProp, showContact.show && showContact.id===index ? {width:'69%'} : null]}
            onPress={()=>{showContact.show?setShowContact({show:false,id:index}):setShowContact({show:true,id:index})}}
            >
            ...
            style={Style.profileImage} />
                <View style={Style.profileBox}>
                  ...
                  <Text style={Style.userBoxHist}>Last  connected: 28.12.2023</Text>
                </View>
            </TouchableOpacity>
            ...
            page={() => navigation.navigate('VideoCall',{cbimg:item.image})}
            bgc="#5FD05D" 
            />
            </View>
            :
